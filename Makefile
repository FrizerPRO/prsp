...
build-debug build-release: build-%: build_%/CMakeCache.txt
	cmake --build build_$* -j $(NPROCS) --target prsp

# Test
.PHONY: test-debug test-release
test-debug test-release: test-%: build-%
	cmake --build build_$* -j $(NPROCS) --target prsp_unittest
	cmake --build build_$* -j $(NPROCS) --target prsp_benchmark
...
start-debug start-release: start-%: build-%
	cmake --build build_$* -v --target start-prsp
...
install-debug install-release: install-%: build-%
	cmake --install build_$* -v --component prsp
...
--in-docker-start-debug --in-docker-start-release: --in-docker-start-%: install-%
	...
	/home/user/.local/bin/prsp \
		--config /home/user/.local/etc/prsp/static_config.yaml \
		--config_vars /home/user/.local/etc/prsp/config_vars.docker.yaml

# Build and run service in docker environment
.PHONY: docker-start-debug docker-start-release
docker-start-debug docker-start-release: docker-start-%:
	$(DOCKER_COMPOSE) run -p 8080:8080 --rm prsp-container make -- --in-docker-start-$*
...
docker-cmake-debug docker-build-debug docker-test-debug docker-clean-debug docker-install-debug docker-cmake-release docker-build-release docker-test-release docker-clean-release docker-install-release: docker-%:
	$(DOCKER_COMPOSE) run --rm prsp-container make $*
