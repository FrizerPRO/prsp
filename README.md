...
3. Give a propper name to your service and replace all the occurences of "prsp" string with that name
   (could be done via `find . -not -path "./third_party/*" -not -path ".git/*" -not -path './build_*' -type f | xargs sed -i 's/prsp/YOUR_SERVICE_NAME/g'`).
